...
2. **纯 FFT + 查表**：每帧仅 672 次查表累加 + 1025 点掩码乘法，43 帧/秒 → 计算量可忽略；
3. **天然流式**：逐帧处理，无需整曲缓冲，延迟与内存都是常数级；
4. **量化简单**：幅度谱 16bit、掩码 1bit，不涉及神经网络量化误差。

### 限制（红线）

| 限制 | 影响 | 应对 |
|------|------|------|
| **分离质量有物理上限** | 人声与吉他频率重叠处不可分（SI-SDR 人声 3.57dB / 伴奏 -0.71dB） | 接受降级；或叠加后处理（按音域/连续性过滤音符） |
| **假设人声单声部** | 合唱、和声、双人说话会失效（只会锁定一个基频） | 场景限定为独唱 |
| **F0 估计误差** | 八度错误、清辅音段漏判会让掩码偏移 | 中值平滑已缓解；可再加连续性约束 |
