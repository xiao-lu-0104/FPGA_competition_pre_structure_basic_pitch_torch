# 人声/吉他分离 —— FPGA 实现方案（DSP 谐波掩码法，备选）

> ⚠️ **这是备选方案**。主方案为 **Spleeter U-Net**（见 `FPGA_分离实现方案.md`），
> 分离质量 SI-SDR 12.98/8.74 dB，但延迟约 3 秒且需外挂 DDR 存 19.6 MB 权重。
> 本 DSP 方案的优势：**零参数、零 DDR、延迟 <100 ms**；代价：质量 5.53/0.20 dB。
> 若比赛硬性要求毫秒级延迟，选本方案。
