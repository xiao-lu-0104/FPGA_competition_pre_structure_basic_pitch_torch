...
| 环节 | 方案 | 关键指标 |
|------|------|---------|
| 分离 | DSP 谐波掩码（FFT → 谐波筛法估 F0 → 平滑梳状掩码 → IFFT） | **零参数**；帧延迟 23ms；ROM 约 4KB |
...
1. **分离从 41M 参数降到 0**：Demucs 不可行的根因是 Transformer 注意力 + 80MB 权重；
   换成 DSP 谐波掩码后**没有任何权重**，每帧只做 FFT + 960 次查表累加。
...
| 限制 | 说明 |
|------|------|
| **F0 估计是瓶颈** | 人声与吉他频率重叠处无法区分；41.6% 的帧 F0 被吉他抢走（主歌段泄漏 0.417，副歌 0.234）——需音色辨识能力（学习模型）才能解决，纯 DSP 已接近上限 |
...
> 分离质量（与 Demucs 参考的相关性）：人声保留 **0.884**、吉他泄漏进人声 **0.209**、
> 人声泄漏进伴奏 **0.276**、人声轨电平 −1.5 dB（已修复"声音小"）。
